% for each set set image, collect bounding boxes from their
% neighbours in the training set (approximate with their nearest neighbour
% in the training set, using an approximate cluster-based search method)

% number of neighbours for each image
num_nb = 50;
% number of nearest clusters for approximate inference
num_nc = 3;
...
pdist = zeros(num_nb, 27743);
pidx = zeros(num_nb, 27743);
[ttpdist, ttpidx] = pdist2(c, tstfeats, 'cosine', 'Smallest',num_nc);
for ii = 1 : 27743
    fprintf('Processing file #%05d...\n', ii);
    mintttpdist = Inf;
    for jj = 1 : num_nc
        clusteridx = (idx == ttpidx(jj,ii));
        fcidx = find(idx == ttpidx(jj,ii));
        clusterfeats = trnfeats(clusteridx, :);
        % get the minimal distance from tstfeats(ii,:) to members of this
        % cluster
        [tttpdist, tttpidx] = pdist2(clusterfeats ,tstfeats(ii,:), 'cosine', 'Smallest', 1);
        if tttpdist < mintttpdist
            tidx = fcidx(tttpidx);
            mintttpdist = tttpdist;
        end
    end
    pdist(:,ii) = tpdist(:,tidx);
    pidx(:,ii) = tpidx(:,tidx);
end
